...
from nodalpulse.db.briefs import (
    get_active_user_ids,
    get_already_enqueued_for_date,
    market_has_subscribers,
)
...
async def _tick(now_ct: datetime) -> None:
    ...
    if now_ct.hour == _CRAWL_HOUR and now_ct.minute < _BRIEF_WINDOW_MIN:  # noqa: SIM102 — keep window + idempotency guards separate for readability
        if not await is_crawl_done_for(today):
            try:
                ...
                if caiso_active or pjm_active:
                    ...
                    logger.info(
                        "No CAISO/PJM subscribers — skipping crawl-ferc/discovery for %s", today
                    )
                ...
                if pjm_active:
                    ...
                    logger.info(
                        "No PJM subscribers — skipping crawl-pjm/imm/pjm-calendar/njbpu for %s",
                        today,
                    )
    ...
    if now_ct.hour == _BRIEF_HOUR and now_ct.minute < _BRIEF_WINDOW_MIN:  # noqa: SIM102 — keep window + idempotency guards separate for readability
...
async def _startup_catchup(now_ct: datetime) -> None:
    ...
    if now_ct.hour >= _CRAWL_HOUR and not await is_crawl_done_for(today):
        try:
            since_date = (today - timedelta(days=1)).isoformat()
            await enqueue("crawl-puct", {"since": since_date}, priority=10)
            await enqueue("crawl-ercot", {"since": since_date}, priority=10)
    ...
    if now_ct.hour >= _BRIEF_HOUR and not await is_brief_done_for(today):
        try:
            count = await _enqueue_briefs_for_date(today)
            await mark_brief_done_for(today)
